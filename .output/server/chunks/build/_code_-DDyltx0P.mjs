import { useSSRContext, defineComponent, ref, onUnmounted, computed, mergeProps, unref, isRef, withCtx, createVNode, createTextVNode, toDisplayString, renderSlot } from 'vue';
...
import { _ as _sfc_main$h } from './Card-T9vkftQH.mjs';
import { cva } from 'class-variance-authority';
import { u as useRoute, a as __nuxt_component_15 } from './server.mjs';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const cStockList = computed(() => {
      ...
      })).filter((stock) => !viewRecentData.value || viewRecentData.value && Number(stock.Time) * 1e3 >= oneHourAgo).filter((stock) => stock.Name.toLowerCase().includes(search.value.toLowerCase())).filter(
        (stock) => !goodTechnical.value || stock.TechnicalDay == "strong_buy" && stock.TechnicalHour == "strong_buy" && // 기술적 시간당 분석
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      const _component_Card = _sfc_main$h;
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "flex flex-col divide-y h-full divide-neutral-300" }, _attrs))}><div class="shrink-0 flex divide-x border-r w-fit divide-neutral-300"><div class="px-4 py-2"><input class="focus:outline-none" type="text"${ssrRenderAttr("value", unref(search))} placeholder="search"></div><div class="px-4 py-2 text-neutral-400 flex items-center gap-2 text-sm">`);
      ...
      _push(ssrRenderComponent(_component_Switch, {
        id: "airplane-mode",
        onClick: ($event) => goodTechnical.value = !unref(goodTechnical)
      }, null, _parent));
      ...
      _push(`</div></div><div class="grow-[0] overflow-hidden flex divide-x h-full divide-neutral-300"><div class="shrink-0 flex flex-col h-full overflow-y-scroll scrollbar-hide"><!--[-->`);
...
//# sourceMappingURL=_code_-DDyltx0P.mjs.map

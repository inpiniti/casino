...
export default defineEventHandler(async (event) => {
  ...
  return await getCurrentStoer("KR");
});
...
export const getCurrentStoer = async (countryCode: string) => {
  ...
  if (!isUpdating) {
    setTimeout(async () => {
      console.log(`[${getKoreaTime()}] new investing`);
      store[String(countryCode)] = await updateStore(String(countryCode));
      console.log(`[${getKoreaTime()}] new investing return`);
    });
  }
  return currentStore;
};
...
async function updateStore(countryCode: string) {
  ...
  try {
    console.log(`[${getKoreaTime()}] new investing fetch start`);

    var formData = new FormData();
    formData.append("country[]", "11");
    ...
    console.log(`[${getKoreaTime()}] new investing parsing start`);
